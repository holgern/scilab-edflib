...
#include "edflib.h"
/* ==================================================================== */
int sci_edf_set_transducer(char *fname)
{
  SciErr sciErr;
 ...
 /* get string */
    sciErr = getMatrixOfString(pvApiCtx, piAddressVarThree,&m3, &n3, NULL, NULL);
  if(sciErr.iErr)
  {
    printError(&sciErr, 0);
    return 0;
    ...
    piLenVarThree = (int*)malloc(sizeof(int) * m3 * n3);
   sciErr = getMatrixOfString(pvApiCtx, piAddressVarThree, &m3, &n3, piLenVarThree, NULL);
                if(sciErr.iErr)
                {
                        printError(&sciErr, 0);
                        return 0;
                ...
                stringData = (char**)malloc(sizeof(char*) * m3 * n3);
                for(i = 0 ; i < n3 * m3 ; i++)
                {
                        stringData[i] = (char*)malloc(sizeof(char) * (piLenVarThree[i] + 1));//+ 1 for null termination
                }

                sciErr = getMatrixOfString(pvApiCtx, piAddressVarThree, &m3, &n3, piLenVarThree, stringData);
                if(sciErr.iErr)
                {
                        printError(&sciErr, 0);
                        return 0;
...
//  csum(&pdVarOne[0],&pdVarTwo[0],&dOut);
   if ((int)pdVarTwo[0]<1){
	      Scierror(999,"edfsignal must be at least 1 !\n");
                return 0; 
   }

if ( edf_set_transducer((int)pdVarOne[0], (int)pdVarTwo[0]-1 , stringData[0]) <0)
{
