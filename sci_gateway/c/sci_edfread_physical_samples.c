...
#include "edflib.h"
/* ==================================================================== */
int sci_edfread_physical_samples(char *fname)
{
  SciErr sciErr;
...
//   }
  ...
  if (pdVarThree[0]<1){
    //m_out=edfhdr.signalparam[(int)pdVarTwo[0]].smp_in_file;
     Scierror(999,"n must be greater then 0.\n");
         return 0; 
  ...
  if (m_out<1){
  Scierror(999,"Could not read signal length.\n");
         //edfclose_file(edfhdr.handle);
         return 0; 
  }
   if ((int)pdVarTwo[0]<1){
...
//  }
 ...
 if (edfread_physical_samples((int)pdVarOne[0],(int)pdVarTwo[0]-1,m_out,dOut) < 0){
    Scierror(999,"Could not read %d samples\n", m_out);
     //edfclose_file(edfhdr.handle);
    return 0;
}
