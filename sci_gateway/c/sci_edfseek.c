...
//   }
  ...
  if (pdVarTwo[0]<1){
    //m_out=edfhdr.signalparam[(int)pdVarTwo[0]].smp_in_file;
     Scierror(999,"the parameter edfsignal must be greater then 0.\n");
         return 0; 
...
//  }
 ...
 if (dOut[0]=edfseek((int)pdVarOne[0],(int)pdVarTwo[0]-1,(int)pdVarThree[0],whence) < 0){
    Scierror(999,"Could not ets the sample position indicator\n");
     //edfclose_file(edfhdr.handle);
    return 0;
}
